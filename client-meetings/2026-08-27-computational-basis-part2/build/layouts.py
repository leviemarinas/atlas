...
def note_card(slide, x, y, w, h, num, title, body, num_color=MARK, num_fill=MARK,
    ...
    # Measure how the title actually wraps at the size it will be drawn. The
    # previous heuristic inferred the line count from the font size, which put
    # a two-line title on top of the first line of its own body text.
    t_lines = max(1, text_lines(title, tw / 914400, t_pt, font=HEAD_FONT, bold=True))
    _, tf = textbox(slide, tx, y + Inches(0.16), tw, Inches(0.30) * t_lines + Inches(0.14))
