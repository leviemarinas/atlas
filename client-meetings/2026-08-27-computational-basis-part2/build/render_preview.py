...
_FALLBACK = next(iter(FONTS.values()))
_cache = {}
...
def font(name, size, bold=False, italic=False):
    ...
    if key not in _cache:
        path = FONTS.get((name, bold)) or _FALLBACK
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        import glob
        found = sorted(glob.glob(os.path.join("..", "outputs", "*.pptx")))
        found = [f for f in found if not os.path.basename(f).startswith("~$")]
        if not found:
            sys.exit("usage: python render_preview.py <deck.pptx>")
        src = found[-1]
