...
$finalDeck = 'C:\Users\josrp\OneDrive\Documents\Atlas\ATLAS_Phase2_Computational_Basis_Demo_WPS_Embedded_Videos.pptx'
...
try {
  ...
  foreach ($item in $items) {
    ...
    $video.Name = 'WPS Embedded Video - ' + $item.Name
    $video.AlternativeText = $item.Name
    $video.AnimationSettings.PlaySettings.PlayOnEntry = 0
    $video.AnimationSettings.PlaySettings.HideWhileNotPlaying = 0
    $video.AnimationSettings.PlaySettings.LoopUntilStopped = 0

    foreach ($shape in $slide.Shapes) {
      if ($shape.HasTextFrame -eq -1 -and $shape.TextFrame.HasText -eq -1) {
        if ($shape.TextFrame.TextRange.Text -like '*EMBEDDED*VIDEO*') {
          $shape.TextFrame.TextRange.Text = '▶ CLICK VIDEO FRAME TO PLAY IN WPS SLIDE SHOW'
        }
      }
    }
  }
