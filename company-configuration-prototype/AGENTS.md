...
## Applicability — who a rule covers

- **One model, in `applicabilityScope.js`.** `{ scope: 'All Employees' | 'Employee Group' | 'Department' | 'Specific Employees', group, department, employees }`, with `coversEmployee`, `coveredEmployees` and `describeScope`. It is pure and React-free so `payrollEngine.js` enforces the very scope the screens edit. `PolicyApplicability.jsx` re-exports it under the names the policy engines already used — it is the panel, never a second definition of "covers".
- **Declare it once, where it is enforced.** Scope used to be written in five places and honoured in three. It now belongs to exactly three, each answering a different question: the **transaction** decides who is in the run, the **operational register** decides which employee carries an item and for how long, and the **configuration's `applicability`** decides which employees a configuration reaches at all. Never add a fourth.
- **The configuration's scope gates every item it governs**, whether or not it binds a formula — `scopeResolverFor` indexes all configurations by code and name, and an item whose configuration does not cover the employee is dropped with an Info exception naming the scope. An item with no configuration (a one-off encoded on the transaction, a loan schedule) answers to nobody's applicability and is never withheld.
- **Migration is permissive on purpose.** The old `employeeGroup` / `subEmployeeGroup` / `employeeNames` triple was never enforced, so `scopeFromLegacyFields` lands every migrated record on "All Employees" and keeps the original wording in `migratedFrom`. Inferring a restriction from a field that never restricted anything would stop paying people who are paid today. Admins narrow scope deliberately.
- **Pipeline assignments carry no scope.** Computational Basis assignments cover only the four computations that own no configuration record — statutory contributions, withholding tax, take-home protection, the retirement benefit. They name a formula and an effective date and nothing else; employee group and frequency live on the configuration and the register.

## Computation binding (Services Information ▸ payroll configurations)

- **A configuration names the formula that computes it.** An Earning, Deduction, Bonus, Allowance, Loan, Pay Rate or Leave configuration carries `computationCode` (a published Computational Basis code) and `computationBindings` (where each of that formula's variables gets its value). `BINDABLE_MODULES` in `computationBindings.js` is the single list of which modules bind and which categories each may draw on — a module becomes bindable by adding an entry there, never by editing `serviceModules.jsx` as well. `withComputationBinding` appends the identical step to every one of them.
- **Four sources, one per variable.** `runtime` (the payroll engine supplies it), `config` (a numeric field on this same record), `reference` (a row of a formula reference source, at the version effective on the payout date), `fixed` (a constant). `runtime` is offered only for a token `ENGINE_SUPPLIED_FIELDS` names; the engine's `runtimeFieldsFor` must have a value for every one of them, and `runtimeContractGaps` is asserted in the suite so the contract cannot drift.
- **Variables come from the whole reference chain.** `bindableTokens` uses `resolvedFields`, so binding a formula that builds on `BAS-001` asks for monthly basic and factor days, not for a daily rate the binder would have to work out by hand.
- **A bound formula returns the amount for the payroll period.** The recurring frequency decides whether an item falls due; it never rescales a bound amount. A bound deduction is still capped at the balance outstanding, and an amount encoded on a transaction is an instruction a binding never overwrites.
- **Half-bound is refused at the screen, not at payroll.** `bindingProblems` blocks the save and names the variable; `runtime` gaps are excluded at configuration time because payroll has supplied nothing yet. At run time a binding that cannot resolve keeps the module's own amount and raises an exception — one broken reference row must not stop a payroll.
- **A bound amount prints its own step.** The step names the code, the version, the expression, the resolved inputs and the source each one came from, so a bound figure is as traceable as a built-in one. Never bypass `evaluateBinding` and compute a bound amount in the engine.
- **The engine stays pure.** `context.serviceConfig` and `context.references` travel in through `buildPayrollContext`; the engine applies a binding, it never goes looking for one. `computationBindings.js` imports only `computationCatalog.js` — importing `computationGovernance.js` would close a cycle back through `payrollRuns.js`.

## Computational Basis governance

- **One central standard, applied per company.** An Atlas standard formula exists once, in Settings ▸ Standard Computation Library. A company never receives a copy. `atlas-standard-computation-applicability-v4` records, per code per company, whether the standard is `applied` and whether it is `Active` there; an absent entry means applied and Active. Never duplicate a standard so a second company can use it.
- **Company scoping.** Company computations, assignments, formula reference sources, published versions and change history are all keyed `<store>:<companyId>`. `readComputationLibrary(companyId)` is the only merged view — company-defined records plus the standards applied to that company, carrying that company's own status. The v3 keys are migrated once per company and the migration is recorded, so a deleted record is not resurrected.
- **Codes are category-driven and locked.** `nextComputationCode(category, library, catalogue)` generates `ERN-001`, `DED-001`, `BON-001` from the category's prefix and the first free three-digit sequence. The code follows the category while the record is new and is read-only from the first save — payroll transactions print it.
- **Categories are controlled, not hard-coded.** The `computation-category` Generic Reference Table owns the category list and its code prefix. `computationCategoryCatalogue()` reads it and falls back to `categoryPrefixes` in `computationCatalog.js`, which is also the table's seed. Users select a category; they do not create one from the Computation screen.
- **New computations are Inactive; description is optional.** A rule stays Inactive while it is built and reviewed, in the company library and in the central library alike. Somebody activates it deliberately. Name is required, description is recommended but never blocking.
- **Delete and deactivate are provable, not advisory.** `computationGuards(record, { context })` returns `canEdit` / `canDelete` / `canDeactivate` with a reason naming the transactions involved. Built-in in a company → activate/deactivate only. Used by a posted transaction → no edit, no delete, in `company` and `standard` context alike, P&A Admin included. Linked to any non-cancelled transaction → no deactivation. Version history or a live assignment → no delete. The way to retire a used code is Inactive.
- **Historical payroll resolves the version it ran on.** Every engine step records `version`, `effectiveDate` and `formulaOwner`; `runPayroll` stores a `computationSnapshot` of every code applied. `libraryForRun(run, current)` returns that snapshot for any run that can no longer be recalculated, so August keeps showing `ERN-002 v1.3` after the live formula becomes v1.4. Never resolve a posted transaction against the current library.
- **Versions are immutable snapshots with their evidence.** `appendVersion` stores the expression, the change diff and the test evidence (inputs, expected, actual, result, tester, timestamp) for each published version. Test evidence is stored, never recomputed when the record is opened.
- **Reference sources keep superseded versions.** `withReferenceVersion` pushes the current rows into `versions` before publishing the new one; `resolveReferenceVersion(reference, asOf)` picks the version whose effective date covers a payout date. Uploading or editing publishes a version — it never overwrites one.
- **Change history carries before and after.** `diffComputation` tracks expression, status, effective date, name, category and description. History entries store a `changes` array and the register renders it, so "who changed what, when, which version" is answerable from the log.
- **Map fields declare their contract.** `fieldMap[code]` carries `owner`, `dataType`, `unit`, `timing` and `missingBehaviour` from the catalogue in `computationCatalog.js`. Any new approved field must supply all five; `fieldOrigin` covers referenced computation codes too.
- **Bulk maintenance.** The register offers per-row checkboxes, select-all-filtered and bulk activate/deactivate. A bulk action reports the codes it could not move and why rather than skipping them silently.
- **Import creates as well as updates.** A CSV row whose code exists updates that company computation; a row whose code is new (or blank, generated from the category) creates one, always Inactive. Atlas standards are never changed by a company import.
...
- The computation importer requires Expression; Code, Name, Category, Status and Effective Date are optional and applied only when present. A known code updates that company computation, an unknown or blank code creates a new Inactive one, and Atlas standards are never touched by import.
