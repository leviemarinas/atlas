...
export function StandardComputationAdmin({ onBack, notify }) {
  ...
  return <div className="page-content standard-computation-admin"><button className="inline-back" onClick={onBack}><ArrowLeft /> Settings</button><div className="page-heading basis-heading"><div><p className="breadcrumb">Settings / Standard Computation Library</p><h1>Standard Computation Library</h1><p className="page-description">Manage Atlas built-in payroll formulas used by Computational Basis assignments and policy engines.</p></div><span className="controlled-badge"><Function weight="duotone" /> Admin source library</span></div><div className="library-admin-notice">{isAdmin ? <Function weight="duotone" /> : <Lock weight="duotone" />}<span>{isAdmin ? <><strong>Single source of truth for standard formulas</strong><small>Add, edit, deactivate, or delete formulas here. Computational Basis consumes the saved library and shows whether each formula is built-in or admin-defined.</small></> : <><strong>Read-only in the client view</strong><small>Switch to the P&amp;A Admin experience in the top bar to add, edit, or delete standard formulas. Client Admins configure assignments and company-specific computations in Computational Basis.</small></>}</span></div><div className="config-toolbar basis-toolbar"><div className="search-box"><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search code, computation, or description..." /><MagnifyingGlass /></div><select className="compact-select" value={category} onChange={event => setCategory(event.target.value)}><option>All categories</option>{[...new Set(computations.map(item => item.category))].map(value => <option key={value}>{value}</option>)}</select><select className="compact-select" value={status} onChange={event => setStatus(event.target.value)}><option>All statuses</option><option>Active</option><option>Inactive</option></select><div className="toolbar-spacer" /><button className="button secondary" onClick={exportCsv}>Export CSV</button>{isAdmin && <button className="button primary" onClick={() => setEditing(null)}><Plus /> Add computation</button>}</div><div className="table-card config-table-card basis-table-card"><table className="config-table basis-table"><thead><tr><th>Code</th><th>Type</th><th>Computation</th><th>Category</th><th>Formula</th><th>Version</th><th>Status</th><th>Action</th></tr></thead><tbody>{visible.map(item => <tr key={item.id}><td><strong>{item.code}</strong></td><td><span className={`computation-source ${item.isBuiltIn !== false ? 'built-in' : 'admin-defined'}`} title={item.isBuiltIn !== false ? 'Built-in standard computation' : 'Admin-defined computation'}><Function weight="duotone" />{item.isBuiltIn !== false ? 'Built-in standard' : 'Admin-defined'}</span></td><td><div className="table-title-cell"><strong>{item.name}</strong><small>{item.description}</small></div></td><td>{item.category}</td><td><code className="table-formula">{item.expression}</code></td><td>{item.version}</td><td><span className={`status-pill ${item.status.toLowerCase()}`}>{item.status}</span></td><td><div className="row-actions always">{isAdmin ? <><button onClick={() => setEditing(item)} aria-label={`Edit ${item.name}`}><PencilSimple /></button><button onClick={() => setDeleting(item)} aria-label={`Delete ${item.name}`}><Trash /></button></> : <span className="row-lock" title="Switch to the P&A Admin experience to edit standard formulas"><Lock weight="duotone" /></span>}</div></td></tr>)}</tbody></table></div><div className="pagination"><span>Displaying <strong>{visible.length}</strong> of {computations.length} formulas</span><span>Admin changes are stored for this company.</span></div>{editing !== undefined && <FormulaAdminModal record={editing || null} existingCodes={computations.map(item => item.code)} onClose={() => setEditing(undefined)} onSave={save} />}{deleting && <DeleteModal record={deleting} onClose={() => setDeleting(null)} onDelete={() => remove(deleting)} />}</div>;
}
