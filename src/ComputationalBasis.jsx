...
const coreComputations = [
  ...
  ['BEN-001', 'Maternity Benefit Reimbursement', 'Benefits', '{{daily_rate}} * 105', 'Computes the standard maternity benefit reimbursement basis.'],
  ['INC-001', 'Commission', 'Incentives', '{{taxable_earnings}} * 0.05', 'Computes commission using the configured eligible earnings.'],
  ['INC-002', 'Piece Rate', 'Incentives', '{{days_worked}} * {{daily_rate}}', 'Computes piece-rate earnings using the configured production unit basis.'],
];

export const categoryCycle = ['Basic Pay', 'Earnings', 'Deductions', 'Government', 'Tax', 'Bonus', 'Year to Date', 'Benefits'];
...
function SummaryCards({ computations, references, assignments }) {
  ...
  return <section className="basis-summary" aria-label="Computational Basis summary">
    <div><Function weight="duotone" /><span><strong>{computations.length}</strong><small>standard computations</small></span></div>
    <div><Table weight="duotone" /><span><strong>{references.length} <em>/ 30 target</em></strong><small>reference tables</small></span></div>
...
function FormulaEditor({ record, onClose, onSave, onTestHistory }) {
  ...
  return <Modal title={`Edit computation · ${record.code}`} onClose={onClose} className="basis-editor-modal">
      ...
      <div className="basis-editor-body">
        {tab === 'formula' && <>
          <div className="basis-form-grid">
            <label>Computation code<input value={draft.code} disabled /></label>
            <label>Computation name<input value={draft.name} onChange={event => setDraft({ ...draft, name: event.target.value })} required /></label>
            <label>Category<select value={draft.category} onChange={event => setDraft({ ...draft, category: event.target.value })}>{categoryCycle.concat(['Payroll Result', 'Separation', 'Incentives']).map(value => <option key={value}>{value}</option>)}</select></label>
          ...
          <section className="formula-builder">
            <div className="formula-builder-heading"><div><h3>Expression builder</h3><p>Use only approved fields and operators. New computation codes cannot be created by clients.</p></div><span className="version-chip">Version {draft.version}</span></div>
        ...
        {tab === 'change' && <div className="change-workspace">
          ...
          <div className="change-summary"><span>Current version <strong>{draft.version}</strong></span><span>Next version <strong>{(Number(draft.version) + 0.1).toFixed(1)}</strong></span><span>Changed by <strong>Client Admin</strong></span></div>
        </div>}
        {error && <div className="basis-error">{error}</div>}
      </div>
      <div className="modal-actions sticky-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary">Validate and save</button></div>
    </form>
  </Modal>;
}
...
function ReferenceEditor({ table: reference, onClose, onSave, onExport }) {
  ...
  return <Modal title={`Manage reference table · ${reference.name}`} onClose={onClose} className="reference-modal">
    <div className="reference-modal-body">
      ...
      <div className="reference-entry-table"><table><thead><tr><th>Key / Range</th><th>Value</th><th>Notes</th><th>Action</th></tr></thead><tbody>
        {draft.entries.map(item => <tr key={item.id}><td><input value={item.key} onChange={event => updateEntry(item.id, 'key', event.target.value)} /></td><td><input value={item.value} onChange={event => updateEntry(item.id, 'value', event.target.value)} /></td><td><input value={item.note} onChange={event => updateEntry(item.id, 'note', event.target.value)} /></td><td><button className="text-danger" onClick={() => removeEntry(item.id)}>Remove</button></td></tr>)}
  ...
  return <div className="page-content computational-page">
    ...
    {tab === 'computations' && <>
      <div className="config-toolbar basis-toolbar">
        ...
        <button className="button secondary" onClick={() => computationUploadRef.current?.click()}><UploadSimple /> Update from template</button>
