...
const agencyDefinitions = {
  ...
  tax: {
    name: 'BIR Withholding Tax', short: 'Tax', prefix: 'BIR',
...
const fieldValue = (agency, row, key) => {
  ...
  if (['benefitCode', 'benefitName', 'frequency', 'excessTreatment'].includes(key)) return row[key] || '—';
...
export function SettingsHub({ onOpen, onOpenReference, onOpenComputationLibrary, onOpenWorkspace }) {
  const cards = [
    ...
    { icon: Receipt, label: 'Statutory Table', detail: 'Tax, SSS, PhilHealth, Pag-IBIG and De Minimis versions', handler: onOpen },
    ...
    { icon: FileText, label: 'Pay Code', detail: 'Payroll code library, classifications and GL mapping', handler: () => onOpenWorkspace('payCodes') },
    { icon: Table, label: 'Payroll Calendar', detail: 'Payout, locking, statutory and billing deadlines', handler: () => onOpenWorkspace('calendar') },
    ...
    { icon: BookOpen, label: 'FAQ / Self-Learning', detail: 'Sanitized searchable FAQ content by module', handler: () => onOpenWorkspace('faq') },
    { icon: Ticket, label: 'Ticketing / Helpdesk', detail: 'Topics, assignment destinations and SLA rules', handler: () => onOpenWorkspace('tickets') },
    { icon: Timer, label: 'Overtime Configuration', detail: 'Canonical overtime types, rates and approvals', handler: () => onOpenWorkspace('overtime') },
  ...
  return <div className="platform-hub page-content">
    ...
    <section className="platform-grid">{cards.map(card => <HubCard key={card.label} icon={card.icon} label={card.label} detail={card.detail} enabled onClick={card.handler} />)}</section>
  </div>;
}

export function PayrollHub({ onOpen, onOpenWorkspace }) {
  const cards = [
    ...
    { icon: FileText, label: 'Payslip Designer', detail: 'Configure branded payslip presentation', handler: () => onOpenWorkspace('payslip') },
    { icon: Table, label: 'Statutory Table', detail: 'View effective tax and contribution tables', handler: onOpen },
    { icon: CheckCircle, label: 'Remittance Monitoring', detail: 'Track government receipts against posted payouts', handler: () => onOpenWorkspace('remittance') },
    { icon: Buildings, label: 'Journal Entry', detail: 'Review balanced payroll accounting entries', handler: () => onOpenWorkspace('journal') },
    { icon: FileText, label: 'Reports', detail: 'Generate payroll, statutory, YTD and audit reports', handler: () => onOpenWorkspace('reports') },
...
export function StatutoryTables({ mode = 'settings', onBack, notify }) {
  ...
  // Tax and statutory tables are P&A-owned; the client side is view/export only.
  const canEdit = mode === 'settings' && isAdmin;
  ...
  const saveEntry = entry => {
    ...
    if (agency !== 'deMinimis' && maximum > 0 && minimum > maximum) {
    ...
    <main className="statutory-main">
      {!tableVersion && mode === 'settings' ? <>
        ...
        {mode === 'settings' && (locked
          ...
          : !canEdit
            ? <div className="statutory-lock-note"><LockKey weight="fill" /><div><strong>Statutory tables are maintained by P&amp;A.</strong><span>Client administrators have view and export access. Switch to the P&amp;A Admin role to publish a version.</span></div></div>
